...
from ..contrib.models import OrderStatusChoices, TrailerTypeChoices, ConditionChoices
...
class OrderFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = Order
        fields = ['status', 'source', 'user', 'extraUser', 'q', 'trailer_type', 'condition', 'origin_state',
                  'destination_state', 'availableDate', 'day', 'period_date_from', 'period_date_to', 'vehicle_id']
            ...
            #         queryset = queryset.filter(quote_dates__quoted__date__range=(start_of_last_week, end_of_last_week))
                ...
                if task_attachments.exists():
                    ...
                    queryset = queryset.filter(
                        id__in=OrderAttachment.objects.filter(
                            link__in=task_attachments.values_list('id', flat=True)).values_list('order_id', flat=True)
                    )
            return queryset
        return queryset
        # if 'today' in value:
